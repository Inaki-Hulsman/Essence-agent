...
with col2:
    ...
    # if not state:
    #     st.info("Aún no hay datos. Empieza la conversación 👈")
    # else:
    #     # ---- CAMPOS PRINCIPALES ----
    #     st.subheader("🎞️ Información básica")

    #     state["title"] = st.text_input("Título", value=state.get("title") or "")
    #     state["genre"] = st.text_input("Género", value=state.get("genre") or "")

    #     # ---- CAMPOS ESTRUCTURADOS ----
    #     st.subheader("🎨 Estilo visual")

    #     visual_style = state.get("visual_style", {})
    #     visual_style["value"] = st.text_area(
    #         "Visual Style",
    #         value=visual_style.get("value") or ""
    #     )

    #     lighting_style = state.get("lighting_style", {})
    #     lighting_style["value"] = st.text_area(
    #         "Lighting Style",
    #         value=lighting_style.get("value") or ""
    #     )

    #     state["visual_style"] = visual_style
    #     state["lighting_style"] = lighting_style

    #     # ---- REFERENCIAS ----
    #     st.subheader("🖼️ Referencias")

    #     references = state.get("references", [])

    #     if references:
    #         for i, ref in enumerate(references):
    #             with st.expander(f"Referencia {i+1}"):
    #                 st.image(ref["url"], use_container_width=True)

    #                 st.write("**Campos asociados:**", ref.get("associated_fields"))

    #                 st.write("**Features extraídas:**")
    #                 st.json(ref.get("extracted_features", {}))
    #     else:
    #         st.write("No hay referencias todavía")

    #     # ---- BOTÓN GUARDAR MANUAL ----
    #     if st.button("💾 Guardar cambios manuales"):
    #         st.success("Cambios guardados en frontend (no persistidos aún en backend)")
